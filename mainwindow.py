...
class MainWindow(QMainWindow):
    ...
    def manualDriveTo(self):
        ...
        if robotino != None:
            if not self.useROS:
                ...
                Thread(target=self.commandServer.goTo,args=[target,robotino.id,"resource"]).start()
        else:
            appLogger.error(
                "[FLEETIAS] Couldnt execute command because robotino is not present"
            )
    ...
    def manualDriveToPos(self):
        ...
        if robotino != None:
            if not self.useROS:
                ...
                # Use ROS Robotino stack
                Thread(target=self.commandServer.goTo,args=[(x,y),robotino.id,"coordinate"]).start()
        else:
            appLogger.error(
                "[FLEETIAS] Couldnt execute command because robotino is not present"
            )
    ...
    def sendCommandAddOffset(self):
        ...
        if robotinoId != 0:
            Thread(target= self.robotinoServer.ROSAddOffset, args=[topic, offset, robotinoId]).start()
        else:
            Thread(target= self.robotinoServer.ROSAddOffset, args=[topic, offset]).start()
    ...
    def sendCommandActivateFeature(self):
        ...
        robotinoId = self.ui.inputRobtinoId.value()
        if robotinoId != 0:
            Thread(target=self.robotinoServer.ROSActivateFeature,args=[
                feature,isEnabled, robotinoId]
            ).start()
        else:
           Thread(target=self.robotinoServer.ROSActivateFeature,args=[
                feature,isEnabled]
